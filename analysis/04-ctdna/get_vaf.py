...
# Author: Jennifer Shelton, Kanika Arora, Minita Shah, William F. Hooper, Timothy R. Chu
...
#        self.source = 'CSQ'
#        self.csq_columns = self.high_conf.header.info[source].description.split()[-1].split('|')
#        self.high_conf_backup = self.load_utf_backup(union_vcf)
    ...
    def read_union_vcf(self, vcf_file):
        ...
        print("Read in " + str(high_conf.shape[0]) + " union records")
        return high_conf
    ...
    def discover_missing(self):
        ...
        for row in self.high_conf.iterrows():
            ...
            self.type = row[1].INFO.split(';')[[idx for idx, s in enumerate(row[1].INFO.split(';')) if s.startswith('TYPE=')][0]].replace('TYPE=', '')
            # search for matching record in VCF
            if not self.is_too_long(self.ref, self.alt):
                match = next(self.bcf_in.fetch(self.chrom, self.pos - 1, self.pos), None)
                if(not match):
                    self.t_bam.create_record(self.chrom, self.pos, self.ref, self.alt, self.type)
                    self.n_bam.create_record(self.chrom, self.pos, self.ref, self.alt, self.type)
